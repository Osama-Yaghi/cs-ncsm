...
!!*******************************
!
!      subroutine inter_2bA_fit(icutnum)
!      use constants
!      !$ use OMP_LIB
!      use paramdef
!      use harmosc
...
!      !file variables
!      character(len=5):: string1,string2,string3,string4,string5
!     &, string6
...
!      call set_default_prec (600)
!      call mpfr_set_default_prec(int(600,8))
!      call initialize_mpfr_constants()
!      temp=zero_mpfr
!      temp3=zero_mpfr
!      e_kd=exp(1._kd)
!      theta_mpfr=fmpfr(theta)
...
!      !loop over J
!      do jrel=1,1!0,min(jrelm,mxeng+1)
!      !***************
!       print *,' doing J=',jrel
!       write(223,*),"***********************"
!       write(223,*),' J=',jrel
...
!       do ij=1,6
!       ! the different couplings of the nn interaction:
!       ! ij=1    S=0 and L1=L2=J
!       ! ij=2    S=1 and L1=L2=J
!       ! ij=3    S=1 and L1=J+1, L2=J+1
!       ! ij=4    S=1 and L1=J+1, L2=J+1
!       ! ij=5    S=1 and L1=J+1, L2=J+1
!       ! ij=6    S=1 and L1=J+1, L2=J+1
!       print*,"**************************"
...
!
!       !*****************
!       ! this determines which on the allowed values of ipot using the antisymmetry constraint:
!       !      (-1)^( L+S+T)= -1
!        jspin=1
!        if(ij==1) jspin=0
!        lr=jrel
...
!        endif
!        ispin=0
!        if((-1)**(jspin+lr)==1) ispin=1
!        if( ispin==0) then
!           !iso-spin=0 => only np allowed
!           if(ipot==2) then
!              ct=1
!           else
!              cycle
...
!!         if(ij/=4 .or. J/=1 .or. ipot/=2) cycle
!         p0=c
!         ci=2._kd/(c0*c0)
!         ck=2._kd/(c2*c2)
!         ni=ci*x0*x0
...
!
!!************
!      end subroutine inter_2bA_fit
!
!**********************
!*******************************

      subroutine inter_2bA_fit_quadrature(icutnum)
      use constants
      !$ use OMP_LIB
      use paramdef
      use harmosc
...
!*      parameter (nstep=500,pc=30.d0,pinf=0.000000001)
     ...
     &, string6
      ...
      integer:: ir,time3,time4,time5,time6
      !parameters 
      integer:: Nd,n_threads,J
      real(kd):: fmbyh2,hbc3,pfmev,rdmavg,bosc,th_kd
      !temporary variables 
      integer:: i,k,ij,i_f,ia,ib,ind,nra,nrb,lra,lrb,jrel,jspin,ispin
      complex(kd)::rrel
      real(kd) ::pi,pk,lr,temp_kd
      real(kd)::c,c0,c2,x0,x2,p0,ci,ck,ni,nk,e_kd,Hmax
      complex(kd),allocatable::H_part1(:,:),H_part2(:,:)
      complex(kd),allocatable,dimension(:,:,:,:,:):: H0
     ...
     & ,v_x_mesh(:)
      real(kd)::theta2,interr,regcut,regpow
      complex(kd),allocatable::u4(:,:,:)
      complex(kd):: wave,integ,vrel,reg
...
!***********************
      v_theta=-0.0_kd
      v_scale=1._kd*sin(2*theta)/sin(2*theta+v_theta)
      e_kd=exp(1._kd)
      rdmavg=2.d0*(fmsp*fmsn)/(fmsp+fmsn)
      fmbyh2=rdmavg/hbc**2
      anu_kd=fmbyh2*hbo ! anu=(m*h_bar*omega)/ (h_bar*c)^2
...
!***********************
      !printing a description
      print*,"called inter_2b_fit_quadrature"
      print*,"$$$$$ theta=",th_kd
!$OMP parallel
!$       n_threads=OMP_GET_MAX_THREADS()
!$OMP end parallel
...
!     &trim(string5)//
!     &"_n3lo_v_n"//trim(string4)//"_range"//trim(string6)//"_J"//
!        v_file_name="2b_fitting/fit_parameters/v_fit4_params_"//
!     &trim(string5)//
!     &"_n3lo_v_n"//trim(string4)//"_range"//trim(string6)//"_J"//
        v_file_name="2b_fitting/lmfit_parameters/v_lmfit_params_v5_"//
     &trim(string5)//
     &"_"//trim(string7)//"_v_n"//trim(string4)//"_range"
     &//trim(string6)//"_J"//
     &trim(string2)//"_ch"//
     &trim(string1)//"_Nf"//trim(string3)//'.fit'
        inquire(file=v_file_name,exist=filetest)
...
!         if (jrel==1 .and. ij==2 .and. ipot==2)
!     &                  c=0
!         if(jrel==1 .and. ij==1 .and. ipot==2  
!     &    ) c=0
!         if (jrel==1 .and. ij==2 .and. ipot/=2
!     &    ) c=0
         p0=c
...
!               if(nra<10.or. nra>nrelm-3) then
!               print*,"***********"
!               print*,nra,lra
!               print*,"numeric",integ
!               print*,"function",H_part1(nra,lra)
!               endif
               integ=0
               do i=1,qn!0,N_arr(nra,lra)
                  !pi=mesh(i,nra,lra)
         ...
         do nra=0,nrelm
            do nrb=0,nrelm
               do lra=max(0,jrel-1),min(jrel+1,jrelm+1)
                  do lrb=max(0,jrel-1),min(jrelm+1,jrel+1)
...
!**********************
      end subroutine inter_2bA_fit_quadrature
